...
const statusStyles: Record<GitHubData["repos"][number]["status"], string> = {
  Active: "bg-primary/12 text-primary",
  Archived: "bg-muted text-muted-foreground",
}

function RepoCard({ repo }: { repo: GitHubData["repos"][number] }) {
  return (
    <a
      href={repo.url}
      target="_blank"
...
export function ProjectsView({ data }: { data: GitHubData }) {
  return (
